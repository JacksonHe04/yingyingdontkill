...
export default function ReadingDeskScene({ books, onSelect, activeTitle }: ReadingDeskSceneProps) {
  ...
  return (
    <div className="relative mt-8 w-full overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-950 via-green-900 to-slate-900 p-6 text-white shadow-2xl shadow-emerald-900/50">
      <div className="absolute inset-0 bg-[radial-gradient(circle,_rgba(34,197,94,0.15)_0%,_transparent_55%)] opacity-80" />
      <div className="relative z-10">
        <p className="text-sm uppercase tracking-[0.5em] text-emerald-100/80">Reading Orbit</p>
        <h3 className="text-2xl font-semibold">书桌上的书 · 可点击的立体书脊</h3>
        <p className="text-sm text-emerald-100/80">挑选一本书，查看简介与链接。</p>
      </div>

      <div className="relative mt-10 h-[320px]">
        <div className="absolute inset-x-0 bottom-0 h-28 rounded-[40px] bg-gradient-to-br from-emerald-200/50 to-lime-200/60 blur-2xl opacity-80" />
          ...
          {featured.map((book, idx) => (
            <motion.button
              ...
              className={`group relative flex h-60 w-20 flex-col items-center justify-end rounded-t-[10px] border border-white/10 bg-gradient-to-b from-white/10 to-white/5 px-3 pb-4 text-center text-xs font-semibold uppercase tracking-widest ${
                activeTitle === book.name ? 'shadow-[0_25px_35px_rgba(52,211,153,0.45)]' : ''
              }`}
              style={{
            ...
            >
              ...
              <span className="text-[10px] text-emerald-100/60">{book.author}</span>
